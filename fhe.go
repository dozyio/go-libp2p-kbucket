...
// Deprecated: Use GenerateKeysForStrategy instead.
// This method will be removed in v2.0.
//
// GenerateKeys generates the client's keypair.
// Note: Rotation keys are not generated by this method.
func (ctx *FHEContext) GenerateKeys() error {
	...
	// Note: EvalMultKeyGen removed - we only use ciphertext×plaintext multiplication
	// so relinearization keys are not needed.

	return nil
}

// Deprecated: Use GenerateKeysForStrategy(PIRStrategyGreedyAdaptive) or similar instead.
// This method will be removed in v2.0.
//
...
func (ctx *FHEContext) GenerateKeysWithRotation() error {
	...
	// Note: EvalMultKeyGen removed - not needed for PIR operations
...
// Deprecated: Use GenerateKeysForStrategy(PIRStrategyGreedyAdaptive) instead.
// This method will be removed in v2.0.
//
// GenerateKeysSparse generates sparse rotation keys optimized for bandwidth.
func (ctx *FHEContext) GenerateKeysSparse() error {
	return ctx.GenerateKeysForStrategy(PIRStrategyGreedyAdaptive)
}

// GenerateKeysForStrategy generates appropriate keys for a PIR strategy.
// This automatically determines whether rotation keys are needed based on the strategy.
func (ctx *FHEContext) GenerateKeysForStrategy(strategy PIRStrategyType) error {
	...
	// Note: We do NOT generate EvalMultKey (relinearization keys).
	// PIR only uses ciphertext×plaintext multiplication, so relinearization is not needed.

	// Determine if rotation keys are needed based on strategy
	switch strategy {
	case PIRStrategyStandard, PIRStrategyPaged:
		// No rotation keys needed - multiply+add operations only
		return nil

	case PIRStrategyPacked, PIRStrategyGreedyAdaptive, PIRStrategyGreedyNormalized:
		// Use sparse rotation keys for bandwidth optimization
		return ctx.generateSparseRotationKeys()

	default:
		return fmt.Errorf("unknown PIR strategy: %s", strategy)
	}
}

// generateSparseRotationKeys generates the minimal rotation key set for bandwidth optimization.
// This is an internal method called by GenerateKeysForStrategy.
func (ctx *FHEContext) generateSparseRotationKeys() error {
	// Base-8 strategy: {1, -1, -8, -64, -512}
	// 5 keys = ~5.63 MB (bandwidth optimized)
	// Provides good balance between key size and rotation efficiency
	indexList := []int32{1, -1, -8, -64, -512}

	err := ctx.CC.EvalRotateKeyGen(ctx.KP, indexList)
	if err != nil {
		return fmt.Errorf("failed to generate rotation keys: %w", err)
	}
...
// RingDim returns the ring dimension of the FHE context
