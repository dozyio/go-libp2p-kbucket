...
// TestFHEPIRRouting_LargeTable tests PIR with ~20 buckets containing ~20 peers each
func TestFHEPIRRouting_LargeTable(t *testing.T) {
	// 1. Setup Context
	ctx, err := NewFHEContext()
	require.NoError(t, err)
	err = ctx.GenerateKeys()
	...
	// Use bucket size of 20 to hold 20 peers per bucket
	rt, err := NewRoutingTable(20, localID, time.Hour, ps, time.Hour, nil)
	require.NoError(t, err)
	rt.EnableFHE(ctx)

	// 3. Populate table with peers for CPLs 0-14
	// (maxCplForRefresh is 15, so we can generate peers for CPLs 0-14)
	// The routing table will split buckets as needed, so we might not get exactly
	// one bucket per CPL, but we'll track which peers are at which CPL
	const targetCPLs = 15 // CPLs 0-14
	const peersPerCPL = 20

	t.Logf("Creating routing table by adding %d peers per CPL (0-%d)", peersPerCPL, targetCPLs-1)

	// Track peers by their actual CPL relative to localID
	peersByCPL := make(map[int][]peer.ID)
	addedCount := 0

	for cpl := 0; cpl < targetCPLs; cpl++ {
		for i := 0; i < peersPerCPL; i++ {
			// Try to add peer with this CPL
			p, err := GenRandPeerIDWithCPL(localID, uint(cpl))
			require.NoError(t, err)

			addrStr := fmt.Sprintf("/ip4/127.0.0.1/tcp/%d", 10000+addedCount)
			addr, err := ma.NewMultiaddr(addrStr)
			require.NoError(t, err)

			ps.AddAddrs(p, []ma.Multiaddr{addr}, time.Hour)

			// Try to add the peer, it's OK if some aren't added due to capacity
			if added, _ := rt.TryAddPeer(p, true, false); added {
				// Verify the CPL is what we expect
				actualCPL := CommonPrefixLen(ConvertPeerID(p), localID)
				peersByCPL[actualCPL] = append(peersByCPL[actualCPL], p)
				addedCount++
			}
		}
	}
	...
	t.Logf("Table created with %d buckets and %d total peers", actualBuckets, rt.Size())
	require.Greater(t, rt.Size(), 100, "Should have added many peers")
	require.GreaterOrEqual(t, actualBuckets, 10, "Should have created multiple buckets")

	// 4. Test PIR queries for various CPLs that have peers
	// We'll test the CPLs that actually have peers
	var testCPLs []int
	for cpl := range peersByCPL {
		if len(peersByCPL[cpl]) > 0 {
			testCPLs = append(testCPLs, cpl)
			if len(testCPLs) >= 5 {
				break
			}
		}
	...
	for _, queryCPL := range testCPLs {
		t.Run(fmt.Sprintf("Query_CPL_%d", queryCPL), func(t *testing.T) {
			// Client Side: Create Query Vector
			queryVec, err := ctx.CreateQueryVector(queryCPL)
			require.NoError(t, err)
			defer func() {
				for _, ct := range queryVec {
			...
			// Server Side: Handle PIR
			start := time.Now()
			encryptedResponse, err := rt.GetBucketPIR(queryVec, ps)
			require.NoError(t, err)
			defer encryptedResponse.Close()
			pirTime := time.Since(start)

			// Client Side: Decrypt
			connectablePeers, err := ctx.DecryptConnectablePeers(encryptedResponse)
			require.NoError(t, err)

			t.Logf("CPL %d: PIR took %v, returned %d peers (expected %d)",
				queryCPL, pirTime, len(connectablePeers), len(peersByCPL[queryCPL]))

			// Verification: Get the bucket for this CPL and check peers
			rt.tabLock.RLock()
			bucketIdx := queryCPL
			...
			// Build expected connectable peers from bucket
			expectedConnectablePeers := make([]ConnectablePeer, 0)
			for _, pInfo := range expectedInternalPeers {
				addrs := ps.Addrs(pInfo.Id)
				if len(addrs) > 0 {
			...
			require.Equal(t, len(expectedConnectablePeers), len(connectablePeers),
				"Should return all peers from bucket")
			...
			for _, expectedPeer := range expectedConnectablePeers {
				require.True(t, peerMap[expectedPeer.ID.String()],
					"Missing expected peer %s in CPL %d", expectedPeer.ID, queryCPL)
			}
		})
	}
}

// TestFHEPIRRoutingPacked_LargeTable tests packed PIR with ~20 buckets containing ~20 peers each
func TestFHEPIRRoutingPacked_LargeTable(t *testing.T) {
	// 1. Setup Context with rotation keys
	ctx, err := NewFHEContext()
	require.NoError(t, err)
	err = ctx.GenerateKeysWithRotation()
	...
	t.Logf("Creating routing table by adding %d peers per CPL (0-%d) - Packed PIR", peersPerCPL, targetCPLs-1)
	...
	// 4. Test Packed PIR queries for various CPLs that have peers
	var testCPLs []int
	for cpl := range peersByCPL {
		if len(peersByCPL[cpl]) > 0 {
			testCPLs = append(testCPLs, cpl)
	...
	for _, queryCPL := range testCPLs {
		t.Run(fmt.Sprintf("PackedQuery_CPL_%d", queryCPL), func(t *testing.T) {
			// Client Side: Create Packed Query
			start := time.Now()
			queryCt, err := ctx.CreateQueryVectorPacked(queryCPL)
			require.NoError(t, err)
			defer queryCt.Close()
			...
			t.Logf("CPL %d: Query creation %v, PIR %v, returned %d peers",
				queryCPL, queryTime, pirTime, len(connectablePeers))
